...
// AddConfiscation is a paid mutator transaction binding the contract method 0x9790f5e9.
//
// Solidity: function addConfiscation(address sender, address asset, uint256 amount) returns()
func (_KeeperRegistry *KeeperRegistryTransactor) AddConfiscation(opts *bind.TransactOpts, sender common.Address, asset common.Address, amount *big.Int) (*types.Transaction, error) {
	return _KeeperRegistry.contract.Transact(opts, "addConfiscation", sender, asset, amount)
}

// AddConfiscation is a paid mutator transaction binding the contract method 0x9790f5e9.
//
// Solidity: function addConfiscation(address sender, address asset, uint256 amount) returns()
func (_KeeperRegistry *KeeperRegistrySession) AddConfiscation(sender common.Address, asset common.Address, amount *big.Int) (*types.Transaction, error) {
	return _KeeperRegistry.Contract.AddConfiscation(&_KeeperRegistry.TransactOpts, sender, asset, amount)
}

// AddConfiscation is a paid mutator transaction binding the contract method 0x9790f5e9.
//
// Solidity: function addConfiscation(address sender, address asset, uint256 amount) returns()
func (_KeeperRegistry *KeeperRegistryTransactorSession) AddConfiscation(sender common.Address, asset common.Address, amount *big.Int) (*types.Transaction, error) {
	return _KeeperRegistry.Contract.AddConfiscation(&_KeeperRegistry.TransactOpts, sender, asset, amount)
}
...
// KeeperRegistryConfiscationAddedIterator is returned from FilterConfiscationAdded and is used to iterate over the raw logs and unpacked data for ConfiscationAdded events raised by the KeeperRegistry contract.
type KeeperRegistryConfiscationAddedIterator struct {
	Event *KeeperRegistryConfiscationAdded // Event containing the contract specifics and raw log
...
// are any more events found. In case of a retrieval or parsing error, false is
// returned and Error() can be queried for the exact failure.
func (it *KeeperRegistryConfiscationAddedIterator) Next() bool {
	// If the iterator failed, stop iterating
	if it.fail != nil {
		return false
	}
	...
	if it.done {
		select {
		case log := <-it.logs:
			it.Event = new(KeeperRegistryConfiscationAdded)
			if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
				it.fail = err
				return false
			}
...
// Error returns any retrieval or parsing error occurred during filtering.
func (it *KeeperRegistryConfiscationAddedIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *KeeperRegistryConfiscationAddedIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}

// KeeperRegistryConfiscationAdded represents a ConfiscationAdded event raised by the KeeperRegistry contract.
type KeeperRegistryConfiscationAdded struct {
	Asset  common.Address
	Amount *big.Int
	Raw    types.Log // Blockchain specific contextual infos
}

// FilterConfiscationAdded is a free log retrieval operation binding the contract event 0xf0a55261e7ddec9f6bc193b27eb01e46468eea35746de9484d28821faa35bc42.
//
// Solidity: event ConfiscationAdded(address asset, uint256 amount)
func (_KeeperRegistry *KeeperRegistryFilterer) FilterConfiscationAdded(opts *bind.FilterOpts) (*KeeperRegistryConfiscationAddedIterator, error) {

	logs, sub, err := _KeeperRegistry.contract.FilterLogs(opts, "ConfiscationAdded")
	if err != nil {
		return nil, err
	}
	return &KeeperRegistryConfiscationAddedIterator{contract: _KeeperRegistry.contract, event: "ConfiscationAdded", logs: logs, sub: sub}, nil
}

// WatchConfiscationAdded is a free log subscription operation binding the contract event 0xf0a55261e7ddec9f6bc193b27eb01e46468eea35746de9484d28821faa35bc42.
//
// Solidity: event ConfiscationAdded(address asset, uint256 amount)
func (_KeeperRegistry *KeeperRegistryFilterer) WatchConfiscationAdded(opts *bind.WatchOpts, sink chan<- *KeeperRegistryConfiscationAdded) (event.Subscription, error) {

	logs, sub, err := _KeeperRegistry.contract.WatchLogs(opts, "ConfiscationAdded")
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			case log := <-logs:
				// New log arrived, parse the event and forward to the user
				event := new(KeeperRegistryConfiscationAdded)
				if err := _KeeperRegistry.contract.UnpackLog(event, "ConfiscationAdded", log); err != nil {
					return err
				}
				event.Raw = log
...
// ParseConfiscationAdded is a log parse operation binding the contract event 0xf0a55261e7ddec9f6bc193b27eb01e46468eea35746de9484d28821faa35bc42.
//
// Solidity: event ConfiscationAdded(address asset, uint256 amount)
func (_KeeperRegistry *KeeperRegistryFilterer) ParseConfiscationAdded(log types.Log) (*KeeperRegistryConfiscationAdded, error) {
	event := new(KeeperRegistryConfiscationAdded)
	if err := _KeeperRegistry.contract.UnpackLog(event, "ConfiscationAdded", log); err != nil {
		return nil, err
	}
